...
> **État : exploration documentaire.** Aucune ligne de code n'a encore été écrite, et rien n'a été testé dans l'Enfusion Workbench. Ce dépôt contient pour l'instant l'analyse préalable : qu'est-ce qui existe déjà nativement dans Reforger, qu'est-ce qu'il faudra construire, et quelle forme ça prendra. Voir [Fiabilité](#fiabilité) avant de vous appuyer sur un détail technique.
...
| `ui` (panneau admin) | À déterminer — probablement en grande partie remplacé par Game Master natif | [Récapitulatif](docs/OMTK_UI_AdminPanel_Reforger_Recap.md) |
...
**Modules du dépôt OMTK encore non audités**, repérés en listant l'ensemble du dépôt source (voir
récapitulatif `ui`, §7) : `respawn_mode`, `view_distance`, `zeus_admins`, `rambo_warn`, `ia_manager`,
`radio_lock` *(déjà couvert)*, `uniform_lock`, `tactical_paradrop`, `vehicles_thermalimaging`,
`difficulty_check` *(déjà couvert)*, `map_exploration`, `3rd-parties`. Plusieurs recoupent déjà
des modules documentés (voir le tableau ci-dessus et le récapitulatif `ui`) — à examiner avant
de considérer l'audit terminé.
...
du jeu et de mods communautaires — sans jamais rien compiler ni exécuter, sauf pour les parties
explicitement marquées « confirmé en pratique » dans les récapitulatifs `kill_logger`, `score_board`
et `warm_up`. Les conséquences pour tout le reste :
...
- chaque récapitulatif se termine par une section « Ce qui reste à valider en pratique » ; c'est
  la partie la plus importante de chaque fichier ;
